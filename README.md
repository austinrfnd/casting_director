...
1. **Default Book Selection:** Pre-populate 200 famous books from the past 30 years in a cached flat file. When the page loads initially, randomly select one of these books to display as a default suggestion. This will:
   - Reduce API calls on initial page load
   - Give users inspiration and a starting point
   - Provide faster initial experience with cached data

2. **Actor Data Caching in Firebase:** Implement a caching layer for actor lookups to improve performance and reduce API costs. When a user enters an actor name:
...
3. **Casting Grade & Perfect Match Suggestions:** Add AI-powered feedback on casting choices after "Make the Movie" is clicked. For each role (including no-name actors):
